...
def special_cal_cov(bam, pos_list, fasta_file_path):
    ...
    def _parse_bam(bam, pos_list, fasta_file_path):
        ...
        for pos in pos_list:
            # print pos
            cov_info = bam_file.count_coverage(reference=pos[0], start=int(pos[1]), end=int(pos[1]) + 1,
                                               quality_threshold=0)
            ...
            cov_info_parsed = [int(i[0]) for i in cov_info]
            # print cov_info_parsed
            cov_info_order = ['A', 'C', 'G', 'T']
            cov_info_dict = dict(zip(cov_info_order, cov_info_parsed))

            ref_base = fastafile.fetch(pos[0], pos[1], pos[1] + 1).upper()
            all_cov_info[pos] = []
            all_cov_info[pos].append((ref_base, cov_info_dict[ref_base]))
...
def parsed_vcf2pos_list(vcf_path):
    ...
    for record in vcf_readed:
        if record.is_snp:
            pos_list.append((record.CHROM, record.POS - 1))
    return pos_list


def count_sample(samples):
    if '-' in samples[0]:
        return list(set([sample.rpartition('-')[0] for sample in samples]))
    else:
        return list(set(samples))


def Add_in_vcf_SO(bam, vcf_path, output_vcf, fasta_file='/home/liaoth/data/hg19/ucsc.hg19.fasta'):
    """
    receive a vcf file and a related bam. Add coverage from bam into vcf and make it a new field.
    For Tumor/Normal only vcf.
