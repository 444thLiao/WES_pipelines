...
def cal_fun(bam_path, bed_file,REF_file='/home/liaoth/data/hg19/ucsc.hg19.fasta'):
    ...
    bed_file = df.from_csv(bed_file, index_col=False, sep='\t',header = None)
    try:
    ...
    total = sum(bed_file.iloc[:,2].values - bed_file.iloc[:,1].values) + len(bed_file)
    pbar = ProgressBar()
    pbar = pbar.start()
    count = 0
    pro_count = 0
    for i in range(len(bed_file)):
        ...
        if sum(sum(j) for j in coverage_ACGT) != 0:
            for base_n in range(start, end + 1):
                ...
                pbar.update(int((float(pro_count) / (total - 1)) * 100))
            if result[-1:] != '\n':
                result += '\n'
