...
class Annovar1(luigi.Task):
    ...
    def run(self):
        os.system("convert2annovar.pl %s --includeinfo -format vcf4 > %s" % (self.input()[0].path, self.output().path))
...
class Annovar2(luigi.Task):
    ...
    def output(self):
        return luigi.LocalTarget(self.input()[0].path.replace('.merged.av','.merged.anno.hg19_multianno.csv'))
    ...
    def run(self):
        os.system(
            "table_annovar.pl %s ~/tools/annovar/humandb/ -buildver hg19 -protocol refGene,phastConsElements46way,genomicSuperDups,esp6500siv2_all,1000g2014oct_all,exac03,snp138,ljb26_all,clinvar_20160302 -operation g,r,r,f,f,f,f,f,f -nastring . --remove --otherinfo --csvout --thread 20 --outfile %s --argument '-exonicsplicing -splicing 25',,,,,,,," % (
                self.input()[0].path, self.output().path.rpartition('.hg19_multianno.csv')[0]))
